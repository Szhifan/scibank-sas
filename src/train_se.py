...
from models import get_tokenizer,ASAG_SentenceEmbeddings
...
def add_training_args(parser):
    ...
    parser.add_argument('--model-name', default='sentence-transformers/all-MiniLM-L12-v2', type=str, help='model type to use')
    ...
    parser.add_argument('--use-multiplication', action='store_true', help='use multiplication in the model')
...
def load_model(args):
    model = ASAG_SentenceEmbeddings(model_name=args.model_name,
            num_labels=len(LABEL2ID[args.label_mode]),
            freeze_layers=args.freeze_layers,
            freeze_embeddings=args.freeze_embeddings,
            use_multiplication=args.use_multiplication)
...
def train_epoch(
    ...
    for epoch in train_iterator:
        train_dataloader = DataLoader(
            ...
            collate_fn=SB_Dataset_SentenceEmbeddings.collate_fn,
            shuffle=True) 
        epoch_iterator = tqdm(
            train_dataloader, 
            desc="Iteration", 
...
def main(args):
    ...
    # Load the dataset
    sbank = SB_Dataset_SentenceEmbeddings(args.label_mode) 
